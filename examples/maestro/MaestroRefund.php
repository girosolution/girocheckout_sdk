...
/**
 * Sample code for GiroCheckout integration of a maestro transaction with refund
 *
 * Call through web server in two steps:
 * 1) Without arguments for initial SALE transaction: http://localhost/MaestroRefund.php, copy displayed reference
 * 2) With reference code from SALE for REFUND, parameter mode=ref (for refund), parameter ref for reference: 
 * http://localhost/MaestroRefund.php?mode=ref&ref=43290-48329043-43289
 * 
 * @filesource
 * @package Samples
...
elseif( $mode == "ref" ) {
  ...
  if( !empty($strReference) ) {
    try {
      $request = new GiroCheckout_SDK_Request(GiroCheckout_SDK_TransactionType_helper::TRANS_TYPE_MAESTRO_REFUND );
      $request->setSecret($projectPassword);
      $request->addParam('merchantId',$merchantID)
              ->addParam('projectId',$projectID)
