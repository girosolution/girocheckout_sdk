...
class GiroCheckout_SDK_Notify
{
   ...
   * @throws GiroCheckout_SDK_Exception_helper if notification is not possible
   */
   ...
   * @throws GiroCheckout_SDK_Exception_helper if an error occurs
   */
  public function parseNotification($params) {
    $Config = GiroCheckout_SDK_Config::getInstance();

    if ($Config->getConfig('DEBUG_MODE')) {
      GiroCheckout_SDK_Debug_helper::getInstance()->logNotificationInput($params);
    }

    if (!is_array($params) || empty($params)) {
      throw new GiroCheckout_SDK_Exception_helper('no data given');
    }
    try {
      $this->notifyParams = $this->requestMethod->checkNotification($params);

      if ($Config->getConfig('DEBUG_MODE')) {
        GiroCheckout_SDK_Debug_helper::getInstance()->logNotificationParams($this->notifyParams);
      }
  ...
  public function sendOkStatus() {
    ...
    if ($Config->getConfig('DEBUG_MODE')) {
      GiroCheckout_SDK_Debug_helper::getInstance()->logNotificationOutput('sendOkStatus');
    }
  ...
  public function sendBadRequestStatus() {
    ...
    if ($Config->getConfig('DEBUG_MODE')) {
      GiroCheckout_SDK_Debug_helper::getInstance()->logNotificationOutput('sendBadRequestStatus');
    }
  ...
  public function sendServiceUnavailableStatus() {
    ...
    if ($Config->getConfig('DEBUG_MODE')) {
      GiroCheckout_SDK_Debug_helper::getInstance()->logNotificationOutput('sendServiceUnavailableStatus');
    }
  ...
  public function sendOtherStatus() {
    ...
    if ($Config->getConfig('DEBUG_MODE')) {
      GiroCheckout_SDK_Debug_helper::getInstance()->logNotificationOutput('sendOtherStatus');
    }
